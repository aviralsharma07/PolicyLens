...
| R20 | Expanded table eval still uses 5-policy-era hard-gate assumptions | Medium | Medium | DSE-022 replaced 5-policy gate with 20-policy dynamic discovery; all gates pass (100% type accuracy, 100% detection recall, 100% header lineage). Remaining limitation: 370 DSE-012 physical labels have empty headers/rows, but current gate passes because header lineage is required only where gold headers exist | — | Mitigated |
...
| R25 | Zero-clause policies block full-corpus extraction coverage | High | Very high | DSE-024 reduced parser-target zero-clause failures from 132 to 0 using safe fallback heading patterns, section-tree rebuild, and one explicit parser-target exclusion for a product-list PDF | — | Mitigated |
| R26 | Missing Product Benefit Tables cause incomplete product facts | High | Very high | DSE-025 source-bundle registry must mark products `missing_pbt`; Product B must not make confident variant-specific recommendations without PBT/table-of-benefits evidence | — | Active |
| R27 | Variant-specific values are flattened into misleading scalar facts | High | Very high | DSE-028 bundle-aware export must carry variant scope, condition scope, source document type, and source quality; Product B must show conditional values as conditional | — | Active |
| R28 | Official source links drift, move, or disappear | High | High | Store source URL, downloaded_at, SHA-256 hash, source domain, and archive/review status for every source document; periodically re-check links | — | Active |
| R29 | Brochure/CIS conflicts with policy wording or PBT | Medium | High | Define source precedence: policy wording and PBT control legal/variant values; CIS/brochure support summaries but conflicts require review before Product B display | — | Active |
| R30 | Same product name maps to multiple UIN versions | High | High | DSE-025 must track UIN, version/effective date, file hash, and source quality per document; mismatches become `uin_mismatch` or `stale_version` | — | Active |
| R31 | Public internet collection is incomplete for old/group/custom products | High | Medium | MVP corpus excludes incomplete source bundles; DSE-026 locks a curated top-10 / top-5 insurer universe instead of all 647 wordings | — | Active |
| R32 | Product B trust damage from overconfident display | Medium | Very high | Product B must display uncertainty, source quality, missing PBT/CIS warnings, and never convert `not_found` into "not covered" | — | Active |
| R33 | MVP insurer shortlist drifts as product pages and sellable product menus change | Medium | High | DSE-027 must re-verify live sellability, URLs, and UIN/version before collecting bundles; DSE-026 rankings are strategy inputs, not permanent truth | — | Active |
| R34 | Older local wording files silently override newer live insurer product versions | High | Very high | DSE-027 latest-version gate now pins a curated 30-product MVP registry against current official insurer sources; DSE-028 must carry `stale_version` and source-quality warnings so Product B never treats historical wording files as current product truth | — | Active |
